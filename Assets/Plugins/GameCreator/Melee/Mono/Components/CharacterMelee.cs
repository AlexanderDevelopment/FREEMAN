...
namespace GameCreator.Melee
{
	...
	public class CharacterMelee : TargetMelee
	{
		...
		private void UpdatePoise()
		{
			...
			if (this.poiseDelayCooldown > float.Epsilon)
				return;
		...
		private void UpdateDefense()
		{
			...
			if (this.defenseDelayCooldown > float.Epsilon)
				return;
		...
		public void PlayAudio(AudioClip audioClip)
		{
			...
			if (this.Blade != null)
				position = this.Blade.transform.position;
		...
		public void SetTargetFocus(TargetMelee target)
		{
			...
			var position = new TargetPosition(TargetPosition.Target.Transform) { targetTransform = target.transform };
		...
		private void DoDamage(CharacterMelee invoker, CharacterMelee receiver)
		{
			...
			invoker.TryGetComponent(out Character invokerCharacter);
			receiver.TryGetComponent(out Character receiverCharacter);

			if (invokerDamageHandler && receiverDamageHandler && invokerCharacter && receiverCharacter)
				receiverDamageHandler.OnReceiveDamage.Invoke(invokerDamageHandler.UnitDamage, invokerCharacter, receiverCharacter);
			else
